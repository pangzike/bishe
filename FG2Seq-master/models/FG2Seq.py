...
class FG2Seq(nn.Module):
    def __init__(self, hidden_size, lang, max_resp_len, path, task, lr, dropout, relation_size, B):
        ...
        if path:
            ...
            self.decoder = DualAttentionDecoder(self.encoder.embedding, lang, hidden_size,
    ...
    def train_batch(self, data, clip, reset=0, ss=1.0):
        ...
        all_decoder_outputs_vocab, all_decoder_outputs_ptr, _, _ = self.encode_and_decode(data, max_target_length, ss,
                                                                                          False)
    ...
    def evaluate(self, dev, matric_best, early_stop=None):
        ...
        for j, data_dev in pbar:
            ...
            for bi, row in enumerate(decoded_fine):
                ...
                if self.task == 'kvr':
                    # compute F1 SCORE
                    _, _, _, single_f1, count = self.compute_prf(data_dev['ent_index'][bi], pred_sent.split(),
                                                                 global_entity_list, data_dev['kb_arr_plain'][bi])
                    F1_pred += single_f1
                    F1_count += count
                    _, _, _, single_f1, count = self.compute_prf(data_dev['ent_idx_cal'][bi], pred_sent.split(),
                                                                 global_entity_list, data_dev['kb_arr_plain'][bi])
                    F1_cal_pred += single_f1
                    F1_cal_count += count
                    _, _, _, single_f1, count = self.compute_prf(data_dev['ent_idx_nav'][bi], pred_sent.split(),
                                                                 global_entity_list, data_dev['kb_arr_plain'][bi])
                    F1_nav_pred += single_f1
                    F1_nav_count += count
                    _, _, _, single_f1, count = self.compute_prf(data_dev['ent_idx_wet'][bi], pred_sent.split(),
                                                                 global_entity_list, data_dev['kb_arr_plain'][bi])
                    ...
                    single_tp, single_fp, single_fn, single_f1, count = self.compute_prf(data_dev['ent_idx_res'][bi],
                                                                                         pred_sent.split(),
                                                                                         global_entity_list,
                                                                                         data_dev['kb_arr_plain'][bi])
                    ...
                    single_tp, single_fp, single_fn, single_f1, count = self.compute_prf(data_dev['ent_idx_att'][bi],
                                                                                         pred_sent.split(),
                                                                                         global_entity_list,
                                                                                         data_dev['kb_arr_plain'][bi])
                    ...
                    single_tp, single_fp, single_fn, single_f1, count = self.compute_prf(data_dev['ent_idx_hot'][bi],
                                                                                         pred_sent.split(),
                                                                                         global_entity_list,
                                                                                         data_dev['kb_arr_plain'][bi])
